...
class SurveyService {
  ...
  async unassignSurvey(id: string, userIds: string[]) {
    const response = await api.delete(`/surveys/${id}/unassign`, { data: { userIds } })
    return response.data
  }

  async getAssignableUsers(id: string) {
    const response = await api.get(`/surveys/${id}/assignable-users`)
    return response.data
  }
