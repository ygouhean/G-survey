# Script PowerShell pour ajouter les champs de réinitialisation de mot de passe
# Windows

Write-Host "🔄 Application de la migration : add-reset-password-fields.sql" -ForegroundColor Cyan
...
$migrationFile = Join-Path $PSScriptRoot "..\server\migrations\add-reset-password-fields.sql"
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "  1. Installer nodemailer : npm install" -ForegroundColor White
    Write-Host "  2. Configurer les variables SMTP dans .env" -ForegroundColor White
    Write-Host "  3. Redémarrer le serveur backend : cd server && npm start" -ForegroundColor White
    Write-Host ""
} else {
    Write-Host ""
